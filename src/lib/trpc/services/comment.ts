...
export const getPostComments = async (input: z.infer<typeof getPostCommentsRequest>) => {
	...
	const postComments = await db
		.select({
			...
			_count: {
				replies: count(replyComments.id).as('replies_count')
			}
		})
		.from(comments)
		.leftJoin(replyComments, eq(comments.id, replyComments.commentId))
...
export const replyComment = async (input: z.infer<ReturnType<typeof replyCommentRequest>>, user: UserPayload) => {
